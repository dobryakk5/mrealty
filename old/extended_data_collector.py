...
class ExtendedDataCollector:
    ...
    def convert_to_cian_format(self, extended_data: Dict[str, Any]) -> Dict[str, Any]:
        """Конвертирует данные W7 в формат совместимый с CIAN parser"""
        ...
        # Определяем source по домену URL
        url = extended_data.get('external_url', '')
        source = "unknown"
        if 'avito.ru' in url:
            source = "avito"
        elif 'cian.ru' in url:
        ...
        # Формат как в CIAN parser
        cian_format = {
            "rooms": extended_data.get('total_room_count'),
            "price": extended_data.get('price'),
            "total_area": extended_data.get('total_square'),
            "living_area": extended_data.get('life_square'),
            "kitchen_area": extended_data.get('kitchen_square'),
            "floor": extended_data.get('storey'),
            "total_floors": extended_data.get('storeys_count'),
            "bathroom": extended_data.get('water_closet_type_name'),
            "balcony": extended_data.get('balcony_type_name'),
            "renovation": extended_data.get('apartment_condition_type_name'),
            "construction_year": extended_data.get('built_year'),
            "house_type": extended_data.get('walls_material_type_name'),
            "ceiling_height": extended_data.get('ceiling_height'),
            "furniture": None,  # W7 не предоставляет детальную информацию о мебели
            "address": address,
            "metro_station": metro_station,
            "metro_time": metro_time,
            "metro_way": None,  # W7 не указывает способ добирания до метро
            "tags": extended_data.get('building_batch_name'),  # Используем серию дома как тег
            "description": extended_data.get('note'),
            "photo_urls": photo_urls,
            "source": source,  # Определяем по домену URL
            "url": url,
            "status": True,  # Данные из активного источника
            "views_today": None  # W7 не предоставляет статистику просмотров
        }
...
async def get_property_by_guid(guid: str) -> Optional[Dict[str, Any]]:
    """Получает данные объявления по GUID в формате CIAN parser"""
    try:
        ...
        # Конвертируем в формат CIAN parser
