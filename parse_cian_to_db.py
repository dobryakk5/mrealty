    ...
    while page <= max_pages:
        try:
            ...
            if new_cards_count == 0:
                ...
                # Если 5 страниц подряд содержат только дубликаты - останавливаемся
                if duplicate_pages_count >= 5:
                    print(f"   🛑 Остановка: 5 страниц подряд содержат только дубликаты")
                    break
            else:
            ...
            # Если карточек меньше MAX_URLS, значит это последняя страница
            if len(cards) < MAX_URLS:
                print(f"   🏁 Достигнут конец результатов (карточек < {MAX_URLS})")
                break
