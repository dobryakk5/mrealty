...
- Поддерживает до 50 метро в одной ссылке (увеличено для больших батчей)
...
Опции:
  ...
  --batch-size N         Автоматически группировать метро в пачки по N штук (5-50 = оптимально)
...
Примеры:
  ...
  python parse_avito_to_db.py --all --batch-size 21   # Все метро группами по 21 (19 пачек)
...
class MetroBatchParser:
    ...
    async def parse_metro_batch(self, metro_list, max_pages, max_cards, start_page=1, use_progress_tracking=False, max_days=0):
        ...
        # 🔧 ДОБАВЛЯЕМ: Информация о формате запуска
        print(f"\n📋 ФОРМАТ ЗАПУСКА:")
        print(f"   • Режим: Обычный парсинг по одному метро")
        print(f"   • Количество метро: {len(metro_list)}")
        print(f"   • Ограничение по дням: {'Да' if max_days > 0 else 'Нет'}")
        if max_days > 0:
            print(f"   • Максимум дней: {max_days}")
        print(f"   • Отслеживание прогресса: {'Включено' if use_progress_tracking else 'Выключено'}")
        
        # Показываем первые несколько метро
        print(f"\n📍 МЕТРО ДЛЯ ПАРСИНГА:")
        for i, metro in enumerate(metro_list[:5]):  # Показываем первые 5 метро
            print(f"   {i+1}. {metro['name']} (ID: {metro['id']})")
        
        if len(metro_list) > 5:
            print(f"   ... и еще {len(metro_list) - 5} метро")
    ...
    async def parse_multiple_metro_single_link(self, metro_ids, max_pages, max_cards, start_page=1, max_days=0):
        ...
        try:
            ...
            # 🔧 ДОБАВЛЯЕМ: Информация о формате запуска
            print(f"\n📋 ФОРМАТ ЗАПУСКА:")
            print(f"   • Режим: Множественные метро одной ссылкой")
            print(f"   • Количество метро: {len(metro_ids)}")
            print(f"   • Ограничение по дням: {'Да' if max_days > 0 else 'Нет'}")
            if max_days > 0:
                print(f"   • Максимум дней: {max_days}")
            print(f"   • Основное метро для отслеживания: {metro_ids[-1]} (последнее)")
    ...
    async def parse_metro_batches(self, metro_batches, max_pages, max_cards, start_page, max_days, use_progress_tracking):
        ...
        # 🔧 ДОБАВЛЯЕМ: Подробная информация о формате запуска
        print(f"\n📋 ФОРМАТ ЗАПУСКА:")
        print(f"   • Режим: Батчи по {len(metro_batches[0]) if metro_batches else 0} метро")
        print(f"   • Всего батчей: {total_batches}")
        print(f"   • Ограничение по дням: {'Да' if max_days > 0 else 'Нет'}")
        if max_days > 0:
            print(f"   • Максимум дней: {max_days}")
        
        # Показываем первые несколько батчей для понимания
        print(f"\n📍 ПЕРВЫЕ БАТЧИ:")
        for i, batch in enumerate(metro_batches[:3]):  # Показываем первые 3 батча
            metro_ids = [metro['id'] for metro in batch]
            metro_names = [metro['name'] for metro in batch]
            print(f"   Батч {i+1}: {metro_ids} - {metro_names}")
        
        if len(metro_batches) > 3:
            print(f"   ... и еще {len(metro_batches) - 3} батчей")
        ...
        for i, metro_batch in enumerate(metro_batches):
            ...
            if success:
                ...
                # 🔧 ИСПРАВЛЕНИЕ: Обновляем прогресс ПОСЛЕ завершения всего батча (параллельная обработка)
                if use_progress_tracking and session_id:
    ...
    try:
        ...
        if args.batch_size > 0:
            ...
            if args.batch_size > 20:
                print(f"   • ⚠️ Большие батчи ({args.batch_size} метро) могут быть менее стабильными")
        print("=" * 60)
