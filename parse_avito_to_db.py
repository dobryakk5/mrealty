...
ЛОГИКА ВРЕМЕННЫХ ОГРАНИЧЕНИЙ МЕЖДУ СЕССИЯМИ:
- При запуске новой сессии проверяется время окончания предыдущей сессии
- Если прошло менее 6 часов с момента завершения последней сессии, новая сессия не запускается
- Это предотвращает слишком частые запуски парсинга и снижает нагрузку на сервер
- Проверка применяется только к новым сессиям (--all и --batch-size)
- Продолжение незавершенной сессии не ограничивается по времени
...
Примеры:
  ...
  python parse_avito_to_db.py --all --batch-size 5 --max-days 7  # Все метро по 5, только свежие (с проверкой 6-часового интервала)
  python parse_avito_to_db.py --all --batch-size 5 --start-page 3  # Все метро по 5, первая пачка с 3-й страницы (с проверкой 6-часового интервала)
"""
    ...
    async def parse_metro_batch(self, metro_list, max_pages, max_cards, start_page=1, use_progress_tracking=False, max_days=0):
        ...
        if use_progress_tracking:
            ...
            if progress and progress['status'] == 'active':
                ...
                # Проверяем временные ограничения перед созданием новой сессии
                print("🕰️ Проверяем временные ограничения для новой сессии...")
                can_start, last_session, hours_since = await check_session_time_limit(1, None, AVITO_SOURCE, 6)
                
                if not can_start:
                    print(f"⚠️ Новую сессия нельзя запустить - прошло менее 6 часов с окончания предыдущей")
                    remaining_hours = 6 - hours_since if hours_since else 6
                    print(f"🕰️ До следующего запуска осталось: {remaining_hours:.1f} часов")
                    print("📝 Повторите запуск позже")
                    return False
    ...
    async def parse_metro_batches(self, metro_batches, max_pages, max_cards, start_page, max_days, use_progress_tracking):
        ...
        if use_progress_tracking:
            ...
            if progress and progress['status'] == 'active':
                ...
                # Проверяем временные ограничения перед созданием новой сессии батчей
                print("🕰️ Проверяем временные ограничения для новой сессии батчей...")
                can_start, last_session, hours_since = await check_session_time_limit(1, None, AVITO_SOURCE, 6)
                
                if not can_start:
                    print(f"⚠️ Новую сессию батчей нельзя запустить - прошло менее 6 часов с окончания предыдущей")
                    remaining_hours = 6 - hours_since if hours_since else 6
                    print(f"🕰️ До следующего запуска осталось: {remaining_hours:.1f} часов")
                    print("📝 Повторите запуск позже")
                    return False
