#!/usr/bin/env python3
"""
Улучшенное обновление avito_id с автоматическим перезапуском браузера
"""
...
class RobustMetroUpdater:
    ...
    def restart_browser(self, cookies_data):
        """Перезапускает браузер и применяет cookies"""
        print("🔄 Перезапускаем браузер...")
        if self.setup_selenium():
            if self.apply_cookies(cookies_data):
                self.session_errors = 0  # Сбрасываем счетчик ошибок
                print("✅ Браузер успешно перезапущен")
                return True
            else:
                print("❌ Не удалось применить cookies после перезапуска")
                return False
        else:
            print("❌ Не удалось перезапустить браузер")
            return False
    
    def detect_metro_station(self, avito_id, cookies_data):
        """Определяет станцию метро по avito_id с автоматическим перезапуском"""
        try:
            ...
            if "invalid session id" in error_msg or "session deleted" in error_msg:
                ...
                print(f"⚠️ Ошибка сессии (попытка {self.session_errors}/{self.max_session_errors}): {error_msg}")
                
                if self.session_errors <= self.max_session_errors:
                    if self.restart_browser(cookies_data):
                        # Повторяем попытку после перезапуска
                        return self.detect_metro_station(avito_id, cookies_data)
                    else:
                        print("❌ Не удалось перезапустить браузер")
                        return None
                else:
                    print(f"❌ Достигнут лимит ошибок сессии ({self.max_session_errors})")
                    return None
            else:
                print(f"❌ Ошибка WebDriver: {error_msg}")
                return None
        except Exception as e:
            print(f"❌ Общая ошибка определения станции avito_id={avito_id}: {e}")
            return None
    ...
    async def process_targeted_metros(self):
        """Обрабатывает только целевые avito_id с автоматическим восстановлением"""
        try:
            ...
            for avito_id in self.all_target_ids:
                ...
                # Определяем станцию по avito_id с автоматическим восстановлением
                metro_info = self.detect_metro_station(avito_id, cookies_data)
    ...
    def save_results(self):
        ...
        try:
            ...
            filename = f"robust_metro_avito_id_results_{time.strftime('%Y%m%d_%H%M%S')}.json"
            ...
            print(f"   Ошибок сессии: {self.session_errors}")
